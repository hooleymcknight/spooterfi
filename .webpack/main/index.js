...
const updateFileContent = async (fileDirectory, songInfo) => {
  await fs.promises.writeFile(`${fileDirectory}${titleFile}`, songInfo.title, {
    flag: 'w'
  }, err => {
    if (err) {
      console.error(err);
  ...
  await fs.promises.writeFile(`${fileDirectory}${artistFile}`, songInfo.artist, {
    flag: 'w'
  }, err => {
    if (err) {
      console.error(err);
...
const {
  getNowPlaying
} = __webpack_require__(/*! ./tools/get-nowplaying.js */ "./src/tools/get-nowplaying.js");
let tray = null;
